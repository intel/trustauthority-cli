...
func TestListApiClientTagsCommandWithInvalidUrl(t *testing.T) {
	...
	invalidUrlTc := []struct {
	...
	}{
		{
			...
			wantErr:     true,
			url:         "bogus\nbase\nURL",
			description: "Test list api client tags using invalid URL",
		},
		{
			...
			wantErr:     true,
			url:         "a/b/c",
			description: "Invalid send request provided for list api client tag command ",
		},
	}
