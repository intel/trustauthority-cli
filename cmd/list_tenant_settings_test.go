...
func TestTenantSettingsError(t *testing.T) {
	...
	invalidUrlTc := []struct {
	...
	}{
		{
			...
			wantErr:     true,
			url:         "bogus\nbase\nURL",
			description: "Test tenant settings using invalid URL",
		},
		{
			args:        []string{constants.ListCmd, constants.TenantSettingsCmd, "-q", "valid-id"},
			wantErr:     true,
			url:         "a/b/c",
			description: "Invalid send request provided for list tenant settings command ",
		},
	}
