...
func TestListServiceOffersCommandWithInvalidUrl(t *testing.T) {
	...
	invalidUrlTc := []struct {
	...
	}{
		{
			...
			wantErr:     true,
			url:         "bogus\\nbase\\nURL",
			description: "Test list service offers using invalid URL",
		},
		{
			args:        []string{constants.ListCmd, constants.ServiceOfferCmd, "-q", "valid-id"},
			wantErr:     true,
			url:         "a/b/c",
			description: "Invalid send request provided for list service offers command ",
		},
	}
